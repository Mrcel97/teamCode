...
This will consist in a few quick steps: 
  ...
  2. No matter whats your role, you can create a Git branch by using the Secret Feature. Lets imagine the initial projet structure is something like:
      ...
      Where "master" is the current project explorer folder. We need to reestructure our projec into a new folder with tha name of the branch. 
      ...
      Now Writer and Auditors are ready to start working on local branches by using the Secret Feature. As an Auditor, lets try to create a 
      ...
      Now, how a Writer do a "Commit" or a "Push"? He's doing a commit and push into "master" every time he modifies a file in the master folder. 
      But, how Auditors make commit and push? They need to ask for Writer Role. Once they are the project Writer. They will change the files of 
      master for their own local versions in the ".develop" folder.

      For those that know Git, the system works as we've got some conflicts when we're trying to push our changes and now we have to manually solve 
...
Incomming Support for:
  ...
  ______________________Invisible Section_____________________________
- TeamCode
- TeamCode
- TeamCode
- TeamCode
